...
class Test:
	...
	def __runner(self, program: str, input: Union[str, int, float, List[str], List[int], List[float]], timeout: float, timeout_factor: float) -> UserProcess:
		full_program: List[str] = [program]
...
class Comparator(ABC):
	...
	def __should_fail(self, user_process: UserProcess, test: Test) -> Result:
		...
		exitcode = user_process.exitcode

		# DEV ERROR: returncode is None
		assert not exitcode is None, "User's process exitcode is None"
		...
		if exitcode != test.exitcode:
			...
			if not empty_stderr:
				...
				print(stderr)
				print('   <-- STDERR')
			return err_exitcode(exitcode, test.exitcode)
	...
	def __should_pass(self, user_process: UserProcess) -> Optional[Result]:
		...
		if exitcode != 0:
			...
			if not empty_stderr:
				...
				print(stderr)
				print('   <-- STDERR')
			return err_should_pass(exitcode)
...
class Tester:
	...
	def add_success(self, name: str, input: Union[str, int, float, List[str], List[int], List[float]], expected: Union[str, int, float, List[str], List[int], List[float]], output_stream: Optional[str] = None, timeout: float = 1.0, categories: Iterable[str] = [], comparator: Optional[Comparator] = None):
	...
	def run(self, program: str, timeout_factor: float, warmup: bool = False) -> Optional[Suite]:
		...
		if warmup:
			for test in self.__tests:
				...
				test.run(program, timeout_factor)
			return None
